...
### 2.2 Importing proteomic data and libraries
...
### 2.3 Data reshaping and cleaning
...
### 3.2 Removing proteins with >20% missing values
...
## 4. Initial random forest model (manual approach)
...
## 5. Structured machine learning workflow (Recipe-Based Approach)
...
### 5.1 Cohort splitting (80/20 Stratified Split)
...
### 5.2 Cross-Validation setup (10-Fold CV)
...
### 5.3 Hyperparameter tuning (mtry Grid)
...
### 6.2 Internal hold-out test performance
...
## 3. Top 25 proteins and importing data
...
### 3.3 Heatmap of significant features
...
### 4.1 Annotation / Feature mapping
...
## 2. Independent test cohort preparation

### 2.1 Importing the external dataset
...
### 2.2 Data reshaping and formatting
...
## 3. Feature alignment with training model

### 3.1 Extracting model predictors
...
## 4. Model application to independent cohort

### 4.1 Generating class predictions and predicted probabilities
...
### 4.3. Calibration plot and Brierer score
...
Calibration performance was assessed in the external cohort using a calibration plot and the Brier score. The predicted probabilities showed strong agreement with observed outcomes, with clear separation between non-Severe and Severe patients. The Brier score was 0.141, indicating good probability accuracy and substantially outperforming random classification (Brier ≈ 0.25 for balanced binary outcomes). Although the calibration curve demonstrates strong alignment between predicted and observed outcomes, the small sample size (n = 10) still limits the precision of these estimates and warrants cautious interpretation.

## 5. Performance evaluation on external cohort

### 5.1 Confusion matrix, accuracy, sensitivity, specificity and Cohen's kappa
...
### 6.1 Generalizability of the proteomic signature  
...
### 6.2 Impact of small sample size (n = 10)  
...
### 6.3 Clinical and biological implications  
...
### 6.4 Need for larger-scale validation  
