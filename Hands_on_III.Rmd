...
The heatmap visualizes the expression patterns of the top 25 proteins identified by the RF model. Although these proteins did not meet the strict FDR-adjusted p-value threshold of 0.05 in the univariate analysis, the heatmap reveals a clear and coordinated proteomic signature.

The hierarchical clustering successfully separates the patients into two main groups that correspond closely with their clinical status. As shown by the top annotation bar, severe patients (pink) exhibit a distinct cluster characterized by the upregulation of several key proteins (indicated by the red blocks), while non-Severe patients (cyan) show generally lower expression of these markers (indicated by the blue blocks). This visual evidence justifies the RF's high AUC of 0.957, the model is identifying a "multivariate signature" where the combined signal of these 25 proteins provides strong discriminatory power that univariate statistical tests failed to capture in this small cohort.
