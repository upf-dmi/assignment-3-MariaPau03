...
---
title: "DMI - Hands on 3: Machine Learning to classify COVID-19 severity"
subtitle: "Classification of COVID-19 clinical outcomes through Random Forest modeling"
...
df_cleaned_final <- df_cleaned %>%
  ...
  slice(1:791) %>%                       # Select rows 1 through 791
  pivot_longer(
...
At this point, I have obtained a tidy data format necessary for ML. As we have ~800 proteins (features) but very few patients, Random Forest model is a great choice because it handles "wide" data well. 
...
As shown in the updated results, the "manual" model (Random Forest with 500 trees) achieved an Out-of-Bag (OOB) error rate of 12.9%. In contrast, the recipe-based model (using the 80/20 split and cross-validation) showed a slightly higher error rate on the internal test set equivalent to 57,14% accuracy.
...
- Recipe-based model performance: This model was restricted to a smaller training set (80% of the cohort) to maintain strict architectural separation. Because it had fewer samples to learn from, its accuracy on the internal 20% hold-out was naturally lower at 57,14%.
...
To further validate the model, the ROC Curve and the Confusion Matrix were analyzed. The model achieves an outstanding AUC (Area Under the Curve) of 0.957. This indicates a roughly ~96% probability that the model will correctly rank a randomly chosen Severe patient higher (in terms of severity risk) than a non-Severe patient.

- Cross-Validation: During the 10-fold cross-validation tuning process, the model reached a peak Accuracy of 86.67% (with mtry=40).

- Internal Testing: On a hold-out test set (the 20% internal division), the model maintained an Accuracy of 57,14%.

(mtry is a tunning parameter that controls the randomness and diversity of the trees in the forest. To decide which protein best separates Severe from non-Severe), it doesn't look at all proteins at once, instead the model randomly picks a subset of proteins, and the size of that subset is mtry, so the tree can only choose the best protein from that specific subset. So mtry = 30 means that the model had enough options to fins strong patterns but enough randomness to stay flexible, reaching a peak accuracy of 0.8667).

On the "unseen" 20% internal division, the model achieved an overall accuracy of 57,14%, correctly classifying 3 non_Severe and 2 Severe patients, misidentifying 1 non_Severe patient as Severe and 1 Severe patient as non_Severe one. Using "non_Severe" as the positive class, the model showed a sensitivity of 75% and a specificity of 33,33%. 


## Conclusion

All in all, the proteomic signature is clearly a strong biological marker for disease severity. The model demonstrates high potential, as shown by the outstanding AUC of 0.957 and the sharp rise in the ROC curve. However, the drop from 86.67% accuracy during training (cross-validation) to 57.14% in the internal test suggests significant overfitting—likely due to the high number of predictors (791 proteins) compared to the small number of samples (24 patients in the training set).

While the manual model shows a lower error rate (12.9%), the recipe-based model remains the more robust choice for Exercise 3. It strictly adheres to the training process architecture by ensuring that the model's generalizability is tested on a truly "unseen" internal division before moving to the external cohort. The recipe model's high AUC confirms that, despite the drop in accuracy on the small internal test set, the underlying proteomic signal remains an excellent tool for separating non-Severe from Severe patients. The focus on proteins alone has concentrated the biological signal, providing a clearer molecular snapshot of COVID-19 severity than the mixed metabolite-protein model.
...
# plot
plot(importance_results, top = 25, main = "Top 25 Protein Predictors")
```
...
df <- df_final_cleaned  # <-- CHANGE THIS to your training dataset object
...
# Volcano plot dataframe to see most variable gene symbols
...
# Volcano plot dataframe to see most variable proteins
volcano_df_proteins <- volcano_df %>%
  left_join(
    top_25_named %>%
      mutate(Protein_ID = tolower(Protein_ID)) %>%
      select(Protein_ID, gene_symbol),
  ...
  # CHANGE THIS LINE: Use Protein_ID for the label instead of gene_symbol
  mutate(Label = Protein_ID) 

# The rest of your ggplot code stays the same!

# See top10 proteins having high log2FC
top_10_significant <- volcano_df %>%
  arrange(desc(abs(log2FC))) %>%     # Sorts from smallest to largest p-value
  slice_head(n = 10)      # Takes the first 10 rows
...
ggplot(volcano_df, aes(x = log2FC, y = negLog10FDR)) +
  ...
  labs(
    title = "Volcano plot for most variable genes: Severe vs non-Severe",
...
ggplot(volcano_df_proteins, aes(x = log2FC, y = negLog10FDR)) +
  geom_point(aes(shape = Significant), size = 3) +
  geom_vline(xintercept = c(-1, 1), linetype = "dashed") +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed") +
  ggrepel::geom_text_repel(
    data = volcano_df_proteins %>% arrange(p_adj) %>% slice(1:10),
    aes(label = Label),
    max.overlaps = 30
  ) +
  labs(
...
The volcano plot (Volcano plot for most variable genes: Severe vs non-Severe) and corresponding data for the Top 25 Random Forest predictors reveal a compelling biological trend, despite not meeting the strict threshold for formal statistical significance. This volcano plot displays the top predictors identified by the model, labeled by their corresponding gene symbols for biological clarity. A "good" log2FC is typically defined as an absolute value greater than 1.0 (which means the protein is either at least twice as abundant or half as abundant in severe patients compared to non-severe ones). In the severe group, the proteins (and its gene symbol) are massively increased (locking at the volcano df :

Protein ID,Gene Symbol,log2FC,Fold Change (linear)
p0dji8,SAA1,3.17,~9.0x higher
p0dji9,SAA2,2.59,~6.0x higher
p02741,CRP,1.81,~3.5x higher
p18428,LBP,1.07,~2.1x higher


Key inflammatory markers such as SAA1 (log2FC = 3.17), SAA2 (log2FC = 2.59) and CRP (log2FC = 1.81) show massive fold changes, indicating they are much more abundant in severe patients. These proteins demonstrated both statistical significance (FDR < 0.05) and substantial effect sizes (log2FC > 1.0). In particular, SAA1 showed a log2FC of 3.17, representing a nearly 9-fold increase in protein abundance in Severe patients, highlighting a massive systemic inflammatory response. Doing some bibliographic research on these markers, SAA1 & SAA2 (Serum Amyloid A1/2) are highly sensitive markers of inflammation. They help recruit immune cells to inflammatory sites but, when chronically high, can contribute to lung tissue damage and hyper-inflammation seen in severe COVID-19. As for CRP, it binds to the surface of dead or dying cells and some bacteria to help the complement system clear them out. High CRP is one of the most consistent predictors of needing mechanical ventilation in COVID-19 patients. Although LBP is usually associated with bacterial infections, it is significantly elevated in severe COVID-19. It indicates a breach in "barrier integrity" (like the gut or lung lining), allowing microbial components to leak into the blood and worsen the inflammatory spiral. So, overall all these inflammatory markers creates a highly accurate fingerprint of a patient in a severe clinical state.


The volcano plot (Volcano plot (Top 25 RF proteins): Severe vs non-Severe) and the Variance important plot (Top 25 Protein Predictors) highlights the fundamental difference between Univariate Statistics (the volcano plot) and multivariate ML (the Variable Importance Plot). The volcano plot looks at each protein one by one, while the varibale importance plot (multivairate) looks at combinations. Moreover, the volcano plot will show every protein that is different. If 10 proteins are all part of the same biological pathway, all 10 will show up as significant on the volcano plot. However, RF doesn't like redundancy. If protein A and protein B provide the exact same information, the RF might only give a high "Importance" score to one of them and ignore the other. So, to sum up, there is a divergence between the top proteins identified by the volcano plot and the RF Variable Importance. This is expected, as the volcano plot identifies individual proteins with the largest magnitude of change (univariate), while the RF identifies the most predictive features in a multivariate context. Proteins that appear important to the model but less significant in the volcano plot likely provide high classification stability or act as crucial 'tie-breakers' when combined with other markers.


Ultimately, this supports the use of ML in this study, as the RF model is able to aggregate the signal from these highly upregulated (but statistically noisy) proteins to achieve an outstanding AUC of 0'957, succeeding where individual t-tests or Wilcoxon tests struggle due to low statistical power.
...
# Intersect (uppercase for matching only)
significant_proteins_upper <- intersect(
  toupper(top_25_named$Protein_ID),
  toupper(colnames(training_data))
)

cat("Matched proteins:", length(significant_proteins_upper), "\n")

# Get the ORIGINAL lowercase column names from training_data
matched_cols <- colnames(training_data)[toupper(colnames(training_data)) %in% significant_proteins_upper]


heatmap_data <- training_data %>%
  select(all_of(matched_cols), group_d)
...
The heatmap visualizes the expression patterns of the top 25 proteins identified by the Random Forest model. While many of these proteins did not meet the conservative FDR-adjusted p-value threshold (Padj<0.05) in univariate testing, the heatmap reveals a clear, coordinated proteomic fingerprint that characterizes disease severity. Hierarchical clustering successfully partitions the cohort into two primary groups that correspond almost perfectly with clinical status. As shown in the top annotation bar:

- Severe Patients (pink): Exhibit a distinct cluster defined by the marked upregulation of key pro-inflammatory markers (represented by the prominent red blocks), including SAA1, SAA2 and CRP. 

- Non-Severe Patients (Cyan): Display a contrasting profile with generally lower expression of these markers (blue blocks).

This visual evidence provides a strong justification for the model's high performance (AUC = 0,957). It demonstrates that the RF is capturing a multivariate signature, a collective signal where the interaction of these 25 proteins provides robust discriminatory power that simple univariate tests failed to capture due to the small cohort size. This reinforces the idea that COVID-19 severity is driven by a systemic proteomic shift rather than changes in isolated individual proteins.
...
Calibration performance was assessed in the external cohort using a calibration plot and the Brier score. The predicted probabilities showed strong agreement with observed outcomes, with clear separation between non-Severe and Severe patients. The Brier score was ~0.16, indicating good probability accuracy and substantially outperforming random classification (Brier ≈ 0.25 for balanced binary outcomes). Although the calibration curve demonstrates strong alignment between predicted and observed outcomes, the small sample size (n = 10) still limits the precision of these estimates and warrants cautious interpretation.
